 ...
 *
 * @version 0.1
 * 2009-07-08 Replaced RichSequence with customized PoorAASequence
 * to allow Serialization
 *
 *
 */
public class WivAAAlignment implements java.io.Serializable {

  java.util.ArrayList<PoorAASequence> seqs;
  private int noOfSequences;
  private int alignmentLength;
  java.util.ArrayList<Integer> sites; // contains the position of the sites
                                      // in the original alignment (for example
                                      // if invariant sites have been removed)
  private int longestName;
  PoorAASequence consensus;
  private String alignmentFilename;

  /**
   *  Creates empty alignment
   */
  public WivAAAlignment()
  {
    seqs = new java.util.ArrayList<PoorAASequence>();
    noOfSequences = 0;
    alignmentLength = 0;
    longestName = 0;
   ...
   *
   */
  public WivAAAlignment(java.util.List<String> names)
  {
    seqs = new java.util.ArrayList<PoorAASequence>();
    longestName = 0;
    ...
    try{
      while (iN.hasNext())
      {
        ...
        seqs.add(new PoorAASequence(seqName, ""));
      }
    }
  ...
  public void writeClustalAlignment(File alignFile) throws java.io.IOException
  {
    ...
    while(block < numberOfBlocks)
    {
      ...
      while(iSp.hasNext())
      {
        PoorAASequence ps = (PoorAASequence) iSp.next();
        bw.write(ps.getName());
  ...
  /**
   * create an alignment from a subset of sequences in the alignment
   * 
   * @param seqNames
   * @return
   * @throws org.biojava.bio.BioException
   */
  public WivAAAlignment getSeqSubset(java.util.ArrayList<String> seqNames)
           throws org.biojava.bio.BioException
   {
     WivAAAlignment aaSubAln = new WivAAAlignment();
      java.util.ListIterator itSeq = this.seqs.listIterator();
      while(itSeq.hasNext())
      {
        PoorAASequence thisSeq = (PoorAASequence) itSeq.next();
        if(seqNames.contains(thisSeq.getName()))
        {
          aaSubAln.addSequence(thisSeq);
          System.out.print("added " + thisSeq.getName() + " to subset\n");
        }
      }
  ...
  /**
   * roll through the alignment site by site and report mutations from the consensus
   *
   *
   *
   * @param bw
   * @throws java.io.IOException
   */
  public void reportMutations(BufferedWriter bw) throws java.io.IOException
  {

    int site = 1;
    while(site<this.alignmentLength)
    {
      ...
      {
        PoorAASequence thisSeq = (PoorAASequence) itSeq.next();
        if(thisSeq.baseAt(site).equals(consensus.baseAt(site)) == false)
        {
          if(header == false)
          {
            header = true;
          ...
          bw.write(thisSeq.getName()
                  + "\t" + thisSeq.baseAt(site)
                  + "\t" + consensus.baseAt(site) + "\n");
        }
  ...
  public void removeInvariantSites()
  {
    ...
    for(int b=1; b<=getAlignmentLength();b++)
    {
      ...
      String currBase = ((PoorAASequence)getSeq().get(0)).subSeq(b, b);
      Boolean invariant = true;
      while(itPoor.hasNext())
      {
        PoorAASequence currSeq = (PoorAASequence) itPoor.next();
    ...
    {
      String variantSeq = "";
      PoorAASequence currSeq = (PoorAASequence) itS.next();
      ...
      PoorAASequence newSeq = new PoorAASequence(currSeq.getName(), variantSeq);
    ...
    while(itSites.hasNext())
    {
      newConsensus = newConsensus.concat(consensus.baseAt((Integer)itSites.next()));
    }
    PoorAASequence newSeq = new PoorAASequence(consensus.getName(), newConsensus);
    consensus = newSeq;
          ...
          boolean ruler) throws java.io.IOException
  {
    ...
    while(iS.hasNext())
    {
      PoorAASequence ps = (PoorAASequence) iS.next();
    ...
    PoorAASequence rS = seqs.get(consSeqNum);
   ...
   */
   public void addSitesAt(int site, java.util.ArrayList<PoorAASequence> colList)
...
/**
   ...
   */
   public java.util.ArrayList<PoorAASequence> getSites(int start_site, int end_site)
   {
     //java.util.ListIterator iSeqs = sequences.listIterator();
     java.util.ListIterator ipSeqs = seqs.listIterator();
     java.util.ArrayList<PoorAASequence> colSeqs
             = new java.util.ArrayList<PoorAASequence>();
     try{
       while(ipSeqs.hasNext())
       {
         PoorAASequence currSeq = (PoorAASequence) ipSeqs.next();
         colSeqs.add(new PoorAASequence(
                 currSeq.getName(),
  ...
  */
  public void findConsensus()
  {
    ...
    consensus = new PoorAASequence("CONSENSUS", cns);
  }
  ...
  public int findSequenceName(String name)
  {
    ...
    while(itN.hasNext())
    {
      PoorAASequence pSeq = (PoorAASequence)itN.next();
   ...
   * @return
   */
  public WivAAAlignment getSubAlignment(int start, int stop)
  {
    WivAAAlignment subAlign = new WivAAAlignment();
    java.util.ListIterator itSeq = seqs.listIterator();
    while (itSeq.hasNext())
    {
      try{
        PoorAASequence seq = (PoorAASequence)itSeq.next();
  ...
  public double[][] bootscanAlignment(
          ...
          )
    {
      ...
      PoorAASequence pa = (PoorAASequence) seqs.get(findSequenceName(refName));
      java.util.ListIterator itSeq = seqs.listIterator();
      int s=0;
      while (itSeq.hasNext())
      {
        PoorAASequence currPAASeq = (PoorAASequence)itSeq.next();
        int site=0; int i=0;
...
public java.util.List getNames()
  {
    ...
    while(itS.hasNext())
    {
      names.add(((PoorAASequence)itS.next()).getName());
    }
   ...
   * @return
   */
  public PoorAASequence getConsensus()
  {
    return consensus;
  }

  /**
   * returns a PoorAASequence list of sequences in the alignment
   * @return
   */
   ...
   * @return
   */
  public PoorAASequence getSeqAt(int row)
  {
    return (PoorAASequence) seqs.get(row);
  }
  ...
  {
    seqs.add(new PoorAASequence(name, seq));
    noOfSequences++;
   ...
   * @throws org.biojava.bio.BioException
   */
  public void addSequence(PoorAASequence ps) throws org.biojava.bio.BioException
  {
    seqs.add(ps);
    noOfSequences++;
    if(seqs.get(seqs.size()-1).getLength() > alignmentLength)
